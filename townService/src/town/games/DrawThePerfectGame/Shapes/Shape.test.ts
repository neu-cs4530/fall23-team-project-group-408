...
describe('Shape', () => {
  ...
  describe('[T1.1] addPixels', () => {
    it('should add a single pixel to the pixelList when given only one', () => {
      expect(shape.pixels.length).toEqual(7);
      shape.addPixels([{ x: 3, y: 9 }]);
      expect(shape.pixels.length).toEqual(8);
    ...
    it('should add a multiple pixels to the pixelList when given multiple', () => {
      expect(shape.pixels.length).toEqual(7);
      shape.addPixels([
        { x: 3, y: 9 },
        { x: 3, y: 10 },
      ]);
      expect(shape.pixels.length).toEqual(9);
      expect(shape.pixels).toEqual([
        { x: 3, y: 3 },
        { x: 3, y: 4 },
        { x: 3, y: 5 },
